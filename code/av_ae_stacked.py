...
def preprocessData():
	...
	filename = '../embeddings_fifty/actionvector_one_per_audiofile.csv' # Modify as necessary
	av_training_data = pd.read_csv(filename) # AV CSV
	...
	filename = '../embeddings_fifty/ESC-50_openl3_music_mel256_6144.npy' # Modify as necessary
	raw_embeddings = np.load(filename, allow_pickle=True) # Imports AEs
	...
	assert(len(x_train_stacked) == 2000)
	assert(len(x_train_stacked[0]) == 6164)
...
(all_x_train, all_y_train, le) = preprocessData()
...
for fold in range(5):
	...
	model.add(Dense(units=800,input_dim = 6164))
